...
export const Account = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-6">
      ...
      >
        {/* Profile card */}
        <div className="flex items-center gap-4 rounded-2xl bg-white p-5 shadow-[0_2px_10px_rgba(0,0,0,0.06)]">
          ...
          <div className="min-w-0">
            <h2 className="truncate text-lg font-bold text-gray-900">{user?.name}</h2>
        ...
        <div className="space-y-2.5">
          {menuRows.map((item, i) => (
            <motion.div
              ...
              className="flex cursor-pointer items-center gap-4 rounded-2xl bg-white p-4 shadow-[0_2px_10px_rgba(0,0,0,0.06)] transition-shadow hover:shadow-[0_10px_25px_rgba(226,55,68,0.12)]"
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand/10">
                <item.icon className="h-5 w-5 text-brand" />
              </div>
              <span className="flex-1 font-semibold text-gray-800">{item.label}</span>
              <BiChevronRight className="h-5 w-5 text-gray-300" />
            </motion.div>
          ))}
        </div>
      </motion.div>
